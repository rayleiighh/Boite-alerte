...
describe('DeviceStatus Component', () => {
  ...
  describe('États de connexion', () => {
    ...
    test('Affiche "Connecté" si ageSeconds < 60', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/Boite à lettre Connecté/i)).toBeInTheDocument();
      });
  ...
  describe('Affichage des statistiques', () => {
    ...
    test('Affiche toutes les stats (Uptime, WiFi, Events, Poids)', async () => {
      const mockResponse = {
        connected: true,
        ageSeconds: 15,
        heartbeat: {
    ...
    test('N\'affiche pas poids si null/undefined', async () => {
      const mockResponse = {
        connected: true,
        ageSeconds: 20,
        heartbeat: {
      ...
      await waitFor(() => {
        // Vérifie que les autres stats sont présentes
        expect(screen.getByText(/Uptime/i)).toBeInTheDocument();
        expect(screen.getByText(/WiFi/i)).toBeInTheDocument();
        expect(screen.getByText(/Events/i)).toBeInTheDocument();
      });

      // Vérifie que "Poids actuel" n'est PAS affiché
      expect(screen.queryByText(/Poids actuel/i)).not.toBeInTheDocument();
    });

    test('Affiche IR libre si beam_state = false', async () => {
      const mockResponse = {
        connected: true,
        ageSeconds: 10,
        heartbeat: {
      ...
      await waitFor(() => {
        expect(screen.getByText(/5\.200 g/i)).toBeInTheDocument();
        expect(screen.getByText(/IR libre/i)).toBeInTheDocument();
      });
    });
    ...
    test('Qualité WiFi - Très bon pour rssi >= -60', async () => {
      const mockResponse = {
        connected: true,
        ageSeconds: 5,
        heartbeat: {
    ...
    test('Qualité WiFi - Bon pour rssi >= -70', async () => {
      const mockResponse = {
        connected: true,
        ageSeconds: 5,
        heartbeat: {
